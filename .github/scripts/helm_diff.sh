...
TMP_ROOT="$(mktemp -d helm_diff_tmp_XXXX)"
...
process_and_diff() {
  ...
  local old_processed="${TMP_ROOT}/old.yaml"
  local new_processed="${TMP_ROOT}/new.yaml"
...
generate_helm_diff() {
  ...
  #####################################
  # Decide: OCI-backed or HTTP repo?  #
  #####################################
  if [[ "$REPO_URL" == oci://* || ( "$REPO_URL" != http://* && "$REPO_URL" != https://* && "$REPO_URL" != file://* ) ]]; then
    ...
    # Start with repoURL, add oci:// if missing
    CHART_REF="$REPO_URL"
...
echo "$RESPONSE" | jq -c '.[] | select(.patch) | 
     ...
     while read -r json; do
        ...
        echo "$APP_ENTRIES" | while IFS="|" read -r APP_NAME NAMESPACE; do
            ...
            generate_helm_diff "$APP_NAME" "$REPO_URL" "$CHART_NAME" "$OLD_VERSION" "$NEW_VERSION" "$DEST_NAMESPACE"
        done
    done
